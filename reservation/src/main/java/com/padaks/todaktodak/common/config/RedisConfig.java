...
//        localdatetime 매핑을 위한 모듈 활성화
    ...
    @Bean
    public MessageListenerAdapter listenerAdapter(@Qualifier("customSignalHandler") SignalHandler redisSubscriber) {
    ...
    @Bean
    @Qualifier("3")
    public RedisTemplate<String, Object> realTimeRedisTemplate(@Qualifier("3") RedisConnectionFactory redisConnectionFactory){
        RedisTemplate<String, Object> redisTemplate = new RedisTemplate<>();
        redisTemplate.setKeySerializer(new StringRedisSerializer());
        redisTemplate.setValueSerializer(new GenericJackson2JsonRedisSerializer(objectMapper()));
        redisTemplate.setConnectionFactory(redisConnectionFactory);
